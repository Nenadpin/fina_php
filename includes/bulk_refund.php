...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        foreach ($bulkDetails as $bulkDetail) {
            $stmtInsert = $conn->prepare("INSERT IGNORE INTO refund (promet, datum, total) VALUES (?, ?, ?)");
            $stmtInsert->execute($bulkDetail);

            if ($stmtInsert->rowCount() > 0) {
                $stmtSelect = $conn->prepare("SELECT datum FROM refund WHERE id = LAST_INSERT_ID()");
                $stmtSelect->execute();
        ...
        foreach ($filteredBulkData as $item) {
            ...
            $qty = intval($item[1]);
            $date = $item[2];
            $conn->prepare("INSERT INTO promet (prod_id, refund, datum) VALUES (?, ?, ?)")->execute([$id, $qty, $date]);
        }
